...
a deployment has and how to safely roll out an update of your application on Kubernetes.
...
1. Test the application as before, by accessing `<public-IP>:<nodeport>`
   ...
   The public IP nor the NodePort have changed, because we didn't change the Service resource.
...
1. If you want to undo your latest rollout, use:
   ...
   Or to retrieve the rollout history,

   ```console
   oc rollout history deploy guestbook
   ```

2. When doing a rollout, you see references to *old* replicas and *new* replicas. The *old* replicas are the original 10 pods deployed when we scaled the application. The *new* replicas come from the newly created pods with the different image. All of these pods are owned by the Deployment. The deployment manages these two sets of pods with a resource called a ReplicaSet. We can see the guestbook ReplicaSets with:
